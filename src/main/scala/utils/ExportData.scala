...
import CommunityDetection.RatedLouvain.VertexData
...
class ExportData {
  ...
  def ExportGraphToCsv(sc: SparkContext, edges: Graph[VertexData, Double], hasWeights: Boolean = false, filePath: String = "./exports/exports.csv", delimiter: String = " ") = {
    deleteFile(sc, filePath)

    edges.edges.repartition(1).map(row => {
      if (hasWeights)
        row.srcId + delimiter + row.dstId
      else
        row.dstId + delimiter + row.dstId + delimiter + row.attr
